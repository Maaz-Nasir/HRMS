    ...
    public class RegionsController : BaseController
    {
        ...
        public IActionResult Listener()
        {
            try
            {
                ...
                int recordsTotal = 0;
                var Data = (from x in dbContext.Regions select x);
                ...
                var resultData = from x in resultList.Where(x => x.SubscriptionId == SessionData.SubscriptionId && !x.IsDeleted)
                                 select new { x.Id, Organization = x.Organization.Name, Country = x.Country.Name, x.Code, x.Name, x.Status,
        ...
        public IActionResult Add()
        {
            ...
            return View("Form", new Regions());
        }
        public Regions GetRecord(int? id)
        {
            UserSessionData UserRecord = GetUserData(this);
            var Record = dbContext.Regions.FirstOrDefault(o => o.Id == id && o.SubscriptionId == UserRecord.SubscriptionId && o.IsDeleted == false);
            if (Record != null)
            {
                DropdownForHRMS(this);
            }
        ...
        public IActionResult Edit(int? id)
        {
            ...
            {
                return Redirect(ViewBag.WebsiteURL + "regions/add");
            }
        }
        ...
        [HttpPost]
        public IActionResult Save(Regions model)
        {
            ...
            try
            {
                if (IsUserLogin(this))
                {
                    ...
                    if (isAddOrUpdate)
                    {
                        ...
                        ajaxResponse.TargetURL = ViewBag.WebsiteURL + "regions";
                    }
                }
                else
                {
        ...
        public JsonResult Delete(int? Id)
        {
            ...
            try
            {
                if (IsUserLogin(this))
                {
                    ...
                    {
                        UserSessionData UserRecord = GetUserData(this);
                        var RecordToDelete = dbContext.Regions.FirstOrDefault(o => o.Id == Id && o.SubscriptionId == UserRecord.SubscriptionId && o.OrganizationId == UserRecord.OrganizationId);
                        if (RecordToDelete != null)
                        {
                            if (RecordToDelete.Cities.Count > 0)
                            {
                            ...
                            else
                            {
                                ...
                                dbContext.Regions.Update(RecordToDelete);
                                dbContext.SaveChanges();
                                ajaxResponse.Success = true;
