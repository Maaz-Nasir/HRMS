...
namespace Hrms.Controllers
{
    public class CitiesController : BaseController
    {
        ...
        public IActionResult GetRegions(int Id)
        {
            ...
            var regions = dbContext.Regions.Where(x => x.CountryId == Id && x.SubscriptionId == UserRecord.SubscriptionId && x.Status == EnumEnableDisable.Enable && x.IsDeleted == false).Select(x => new { x.Id, x.Name }).ToList();
            return Json(regions);
        }
        [HttpPost]
        public IActionResult Listener()
        {
            try
            {
                ...
                int recordsTotal = 0;
                var Data = (from x in dbContext.Cities select x);
                ...
                var resultData = from x in resultList.Where(x => x.SubscriptionId == SessionData.SubscriptionId && !x.IsDeleted)
                                 select new { x.Id, Organization = x.Organization.Name, Country = x.Country.Name, Region = x.Region.Name, x.Code, x.Name, x.Status,
        ...
        public Cities GetRecord(int? id)
        {
            UserSessionData UserRecord = GetUserData(this);
            var Record = dbContext.Cities.FirstOrDefault(o => o.Id == id && o.SubscriptionId == UserRecord.SubscriptionId && o.IsDeleted == false);
            if (Record != null)
            {
                DropdownForHRMS(this);
            }
        ...
        [HttpPost]
        public IActionResult Save(Cities model)
        {
        ...
        public JsonResult Delete(int? Id)
        {
            ...
            try
            {
                if (IsUserLogin(this))
                {
                    ...
                    {
                        UserSessionData UserRecord = GetUserData(this);
                        var RecordToDelete = dbContext.Cities.FirstOrDefault(o => o.Id == Id && o.SubscriptionId == UserRecord.SubscriptionId && o.OrganizationId == UserRecord.OrganizationId);
