    ...
    public class ExpenseTypesController : BaseController
    {
        ...
        public IActionResult Listener()
        {
            try
            {
                ...
                int recordsTotal = 0;
                var Data = (from x in dbContext.ExpenseTypes select x);
                ...
                var resultData = from x in resultList.Where(x => x.SubscriptionId == SessionData.SubscriptionId && !x.IsDeleted)
                                 select new { x.Id, Organization = x.Organization.Name, x.Name,
        ...
        public IActionResult Add()
        {
            ...
            return View("Form", new ExpenseTypes());
        }
        public ExpenseTypes GetRecord(int? id)
        {
            UserSessionData UserRecord = GetUserData(this);
            var Record = dbContext.ExpenseTypes.FirstOrDefault(o => o.Id == id && o.SubscriptionId == UserRecord.SubscriptionId && o.IsDeleted == false);
            if (Record != null)
            {
                DropdownForHRMS(this);
            }
        ...
        public IActionResult Edit(int? id)
        {
            ...
            {
                return Redirect(ViewBag.WebsiteURL + "expensetypes/add");
            }
        }
        ...
        [HttpPost]
        public IActionResult Save(ExpenseTypes model)
        {
                ...
                if (IsUserLogin(this))
                {
                    ...
                    if (isAddOrUpdate)
                    {
                        ...
                        else
                        {
                            ...
                            dbContext.ExpenseTypes.Update(model);
                            ajaxResponse.Success = true;
                            ajaxResponse.Message = "Record Updated Successfully";
                        }
                        dbContext.SaveChanges();
                        ajaxResponse.TargetURL = ViewBag.WebsiteURL + "expensetypes";
                    }
                }
                else
                {
        ...
        public JsonResult Delete(int? Id)
        {
            ...
            try
            {
                if (IsUserLogin(this))
                {
                    ...
                    {
                        UserSessionData UserRecord = GetUserData(this);
                        var RecordToDelete = dbContext.ExpenseTypes.FirstOrDefault(o => o.Id == Id && o.SubscriptionId == UserRecord.SubscriptionId && o.OrganizationId == UserRecord.OrganizationId);
                        if (RecordToDelete != null)
                        {
                            if (RecordToDelete.Expenses.Count > 0)
                            {
                            ...
                            else
                            {
                                ...
                                dbContext.ExpenseTypes.Update(RecordToDelete);
                                dbContext.SaveChanges();
                                ajaxResponse.Success = true;
